...
class ItemBasedPredictor:
    def __init__(self, min_values=0, threshold=0):
        self.min_values = min_values
        self.threshold = threshold
        self.similarities = defaultdict(dict)  # {movie_id: {other_movie_id: similarity}}
        self.movie_ratings = defaultdict(dict)  # {movie_id: {user_id: rating}}
        self.global_avg = None  # Povprečna ocena vseh filmov
        self.testCounter = 0  # Število testov (za sledenje podobnostim)

    def fit(self, X):
        # Zbiranje ocen za vsak film
        for user_id, movie_id, rating, _ in X.data:
            self.movie_ratings[movie_id][user_id] = rating
        
        # Izračun globalnega povprečja ocen
        all_ratings = [rating for movie_ratings in self.movie_ratings.values() for rating in movie_ratings.values()]
        self.global_avg = np.mean(all_ratings)
        
        movie_ids = list(self.movie_ratings.keys())
        
        # Izračun podobnosti med filmi
        for i, movie1 in enumerate(movie_ids):
            for movie2 in movie_ids[i+1:]:
                sim = self.similarity(movie1, movie2)
                self.testCounter += 1
                ...
                if sim > 0:
                    self.similarities[movie1][movie2] = sim
                    self.similarities[movie2][movie1] = sim  # Simetrično shranjevanje podobnosti

    def predict(self, user_id):
        predictions = {}
        
        for movie1 in self.movie_ratings:
            if user_id not in self.movie_ratings[movie1]:
                # Predvidimo oceno za film, ki ga uporabnik še ni ocenil
                numerator = 0
                denominator = 0
                
                for movie2 in self.movie_ratings:
                    if user_id in self.movie_ratings[movie2] and movie1 != movie2:
                        similarity = self.similarities.get(movie1, {}).get(movie2, 0)
                        if similarity > 0:
                            numerator += similarity * self.movie_ratings[movie2][user_id]
                            denominator += similarity
                
                if denominator > 0:
                    predictions[movie1] = numerator / denominator
                else:
                    predictions[movie1] = self.global_avg  # Če ni podobnosti, uporabimo globalno povprečje
